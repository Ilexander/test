...
<link rel="stylesheet" type="text/css" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css"/>
  ...
  <div class="services__wrapper">
    ...
    @if(! \Illuminate\Support\Facades\Auth::user()->isAdmin())
        @foreach($categories as $category)
            <div class="services__projects mt-3" >
              <div class="services__projects--body _section-block">
                  <div class="anch" id="{{$category->name}}"></div>
                                                    ...
                                                    <div class="project-block__popup--body _popup-body">
                                                        ...
                                                        <div class="project-block__popup--content">
                                                            <ul>
                                                                @php
                                                                    $list = '';
                                                                    $desc = explode('if you want a special price - write to tickets', $service->desc);
                            ...
                            <tr id="service-{{$service->id}}" class="category-item">
                                <td class="p-3 pb-4 pt-4">
                                    <div class="category__table--checkbox ps-3 mb-2">
                                        <input
                                            type="checkbox"
                                            name="category-checkbox-{{$service->id}}"
...
<script>
    ...
    {
        $('#serviceFormLabel').html('{{ __('locale.service.form.edit_service') }}');
        ...
        $('#serviceCreateForm').attr('action', '{{route('admin.service.update', ['language' => Config::get('language.current')])}}');
    ...
    {
        $('#serviceFormLabel').html('{{ __('locale.service.form.add_service') }}');
        $('#serviceFormMethod').html('');
        $('#serviceFormServiceId').html('');
        $('#serviceCreateForm').attr('action', '{{route('admin.service.create', ['language' => Config::get('language.current')])}}');
        $('#pkgname').val('');
        $('#descr').val('');
        $('#rate').val('');
        $('#serviceFormOriginalPrice').val('');
